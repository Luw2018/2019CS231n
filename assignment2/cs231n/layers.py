...
#     dx_hat = dout * gamma
#     dx_var = np.sum(dx_hat * (x - x_mean) * (-0.5) * (x_var + eps) ** - 1.5, axis = 0)
#     dx_mean = np.sum(dx_hat * (-1 / np.sqrt(x_var + eps)), axis = 0)
...
#             dx[:, :, s*hh:s*hh+pool_height, s*ww:s*ww+pool_width] = dx_idx * dout_slice[:, :, np.newaxis, np.newaxis]
            dx[:, :, s*hh:s*hh+pool_height, s*ww:s*ww+pool_width] = dx_idx * dout_block[:, :, np.newaxis, np.newaxis]
